...
//
//
//  LUXSim2evtMethods.cc
//
//  A support file containing various methods used in LUXSim2evt
//  
//////////////////////////////////////////////////////////////////////////////
//  
//  Change Log:
...
void determineSensitiveVolume(char* InputCommands,
    ...
    string record_level_string = "/LUXSim/detector/recordLevel";
    ...
    if( !record_photocathode ) {
        ...
        cerr << "Ensure the LUXSim macro has a record level set in" << "\n";
        cerr << "PMT_PhotoCathode by adding the following:\n\t";
        cerr << "/LUXSim/detector/recordLevelThermElec PMT_PhotoCathode 3" << endl;
        exit(0);
    }
...
int* makePMTLookupTable(char*DetCompo, int *PMTLookupTable, bool oldPMTStyle) {
    ...
    */
    ...
    // will match pmt number 58 to the G4 volume number 357 (created when LUXSim
    // was run).
...
void createPositionLookup(int* pmt_relations) {
  ...
  // of pmts used in LUXSim to the "pizza" like arrangement used
  // in the LUX detector (as of Run02). If you supply a LUXSim pmt
...
double RandN() {
    double x1, x2, w, y1;
    static double y2;       // Static declaration will exist in mem until exit   
    static int phase = 0;   //
...
double SPE_template(double x, double *par) {
  double small = 1e-6;
  ...
  f1 = isfinite(f1) ? f1: 1e300;
  double f2 = exp(-0.5*t*t/ss + 0.5*mu2*mu2/ss);
  f2 = isfinite(f2) ? f2: 1e300;
...
int GetPhotonTypeIndex(double kinetic_energy) {
  // NB!!
  // THe unit of "double kinetic_energy" is in keV
  // NB!!
  ...
  if(abs(kinetic_energy - 1e3) < 1) // 1 MeV - S1
    return 0;
  if(abs(kinetic_energy - 2e3) < 1) // 2 MeV - S2
    return 1;
  if(abs(kinetic_energy - 3e3) < 1) // 3 MeV - e-train/super-train
    return 3;
  if(abs(kinetic_energy - 4e3) < 1) // 4 MeV - liquid e- noise
    return 4;
  if(abs(kinetic_energy) < .1)      // ~  eV - Cerenkov light.
    return 5;
  cerr <<
  "I found a photon I couldn't identify the source of. It wasn't an S1 or S2 "
  << "or noise in the traditional sense... bug Matthew about this." << endl;
  return -1;  // Error case. Sure to cause a seg fault.
}
