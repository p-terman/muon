...
#include "LUXSim2evtBaseline.hh"

using namespace std;

// Constructors
LUXSim2evtPulse::LUXSim2evtPulse() {}
// Destructors
LUXSim2evtPulse::~LUXSim2evtPulse() {
  //this->data.clear();
}
...
// "Get" functions.
//double LUXSim2evtPulse::GetGain(){return gHeight * (1000. / -7.26) * 16.0;}
double LUXSim2evtPulse::GetGain(){return gArea;}
double LUXSim2evtPulse::GetSpheSigma(){return sphe_sigma;}
double LUXSim2evtPulse::GetRiseTime(){return riseTime;}
double LUXSim2evtPulse::GetFallTime(){return fallTime;}
double LUXSim2evtPulse::GetAmplitude(){return gHeight;}
unsigned int LUXSim2evtPulse::GetLocalEventID () {return local_event_id;}
unsigned int LUXSim2evtPulse::GetPulseNumber () {return pulse_number;}
unsigned int LUXSim2evtPulse::GetChannelNumber () {return channel_number;}
unsigned int LUXSim2evtPulse::GetPulseHeader () {return pulse_header;}
unsigned long long LUXSim2evtPulse::GetPulseTimestamp () {
    return pulse_timestamp; }
unsigned int LUXSim2evtPulse::GetPulseLength () {return pulse_length;}
unsigned int LUXSim2evtPulse::GetBaseline () {return baseline;}
vector<unsigned short>& LUXSim2evtPulse::GetData () {return data;}
double LUXSim2evtPulse::GetTimeOffset () {return rTimeOffset;}
signed int LUXSim2evtPulse::GetPulseStart() {return pulse_start;}

// "Set" functions.
//void LUXSim2evtPulse::SetGain(double gain){gHeight=gain*(-7.26 / 1000.) / 16.0;}
void LUXSim2evtPulse::SetGain(double gain){gArea=gain;}
void LUXSim2evtPulse::SetSpheSigma(double sphe_sig){sphe_sigma = sphe_sig;}
void LUXSim2evtPulse::SetRiseTime(double tRise){riseTime = tRise;}
void LUXSim2evtPulse::SetFallTime(double tFall){fallTime = tFall;}
void LUXSim2evtPulse::SetAmplitude(double amp){gHeight = amp;}
void LUXSim2evtPulse::SetLocalEventID (unsigned int EID) {local_event_id=EID;}
void LUXSim2evtPulse::SetPulseNumber (unsigned int pn) {pulse_number=pn;}
void LUXSim2evtPulse::SetChannelNumber (unsigned int cn) {channel_number=cn;}
void LUXSim2evtPulse::SetPulseHeader (unsigned int ph) {pulse_header=ph;}
void LUXSim2evtPulse::SetPulseTimestamp (unsigned long long pt) {
    pulse_timestamp=pt;}
void LUXSim2evtPulse::SetPulseLength (unsigned int pl) {pulse_length=pl;}
void LUXSim2evtPulse::SetBaseline (unsigned int bl) {baseline=bl;}
void LUXSim2evtPulse::SetData (vector <unsigned short> d) {data=d;}
void LUXSim2evtPulse::SetTimeOffset (double rto) {rTimeOffset=rto;}
void LUXSim2evtPulse::SetPulseStart(signed int ps) {pulse_start=ps;}


// The good stuff

//============================================================================
//            LUXSim2evtPulse::GeneratePrePods
//============================================================================
vector<vector<double> > LUXSim2evtPulse::GeneratePrePods(
...
//============================================================================
//            LUXSim2evtPulse::GeneratePulseVec
//============================================================================
void LUXSim2evtPulse::GeneratePulseVec(std::vector<double> &photon_times) {
    ...
    vector<double> sphe_variations;
		double parameters[5];
		parameters[0] = gHeight;
		parameters[2] = riseTime;
		parameters[3] = fallTime;
		parameters[4] = 6;
    ...
    */
    ...
    for(size_t photon=0; photon < photon_times_size; photon++) {
        ...
        if(sphe_variations[photon] < 0 || sphe_sigma == 0)
    ...
    double voltage_sum;
		...
		// Grab sample of pre-generated baseline trace for timing window
    vector <double> baseline_trace;
    int timing_window = (loop_end - loop_start); //Convert from samples to seconds
    int baseline_samples = (sizeof(baseline_voltages)/sizeof(*baseline_voltages));
    int starting_index = floor(Rand()*baseline_samples);
    while((timing_window+starting_index) > baseline_samples) {
      starting_index = floor(Rand()*baseline_samples);
    }

    for(int sample = starting_index; sample < (starting_index+timing_window); sample++) {
      baseline_trace.push_back(baseline_voltages[sample]);
    }	

		// Add up photon contributions to each sample in timing window
    for(int sample= loop_start; sample < loop_end; sample++ ){
        ...
        for(unsigned int photon=0; photon < photon_times_size;photon++){
            parameters[1] = photon_times[photon];
            if(time - parameters[1] > 300 || time - parameters[1] < -300)
                continue;
            voltage_sum += 
								SPE_template(time, parameters) * sphe_variations[photon];
        }
				
				// Add baseline to pulse vector
				noise = baseline_trace[sample-loop_start];
				voltage_sum += noise;
...
//============================================================================
//            LUXSim2evtPulse::SeparatePulses
//============================================================================
std::vector<LUXSim2evtPulse*> LUXSim2evtPulse::SeparatePulses() {
    ...
    std::vector<LUXSim2evtPulse*> pulse_vec;
    ...
    for(long long i=pretrigger; i< loop_start; i++) {
        ...
        if(found_start && found_end) {
            ...
            //baseline = 15564;
            LUXSim2evtPulse* new_pulse = new LUXSim2evtPulse();
    ...
    for(size_t pod_ind=1; pod_ind<pulse_vec.size(); pod_ind++) {
      LUXSim2evtPulse* p = pulse_vec[pod_ind];
      LUXSim2evtPulse* p_minus_1 = pulse_vec[pod_ind-1];
      ...
      if(end_of_first >= start_of_second) {
        ...
        // Now delete the old LUXSim2evtPulse instance
        delete p;
